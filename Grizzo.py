...
def loadSql():
    ...
    cursor.execute("If Not Exists(select server_id from server where code='"+guildID+"') Begin insert into server_id values ('"+guildID+"') End")
...
#display pins
def pins(ctx, index):
    output = ctx.author.mention+", pinned messages: (date in UTC)```"
    for i in range(int(index), len(arrayPinned)):
        if len(arrayPinned[i][0].message.content) > 300:
            messageContent = arrayPinned[i][0].message.content[0:299] +"... use !pin "+str(i)+" to see full message"
        else:
            messageContent = arrayPinned[i][0].message.content
        outputTest= output+"\n{}: {} {}, 📌 by {}\n\t{}\n".format(str(i),arrayPinned[i][0].message.author.name, arrayPinned[i][0].message.created_at.strftime("%m-%d-%Y %I:%M %p"),arrayPinned[i][1].name, messageContent)
        if len(outputTest)<1800:
            output=outputTest
        else:
            output+="\nShowing {} out of {} pins".format(str((i+1)), len(arrayPinned))
            break
    return output+"\nTo jump to a specific pin use command: !gotopin <ID> \nTo view next batch of pins, use command !pin <ID to start at> ```"


#display pins
def pin(ctx, id):
    embed = discord.Embed( colour=discord.Colour(0x4771c0), description=" ```{}```".format(arrayPinned[int(id)][0].message.content),
                          timestamp=arrayPinned[int(id)][0].message.created_at)
    embed.set_author(name="{}".format(arrayPinned[int(id)][1].name), url="https://discordapp.com",
                     icon_url="{}".format(arrayPinned[int(id)][1].avatar_url))
    embed.set_footer(text="Grizzo")
    embed.add_field(name=":pushpin:  by {}".format(arrayPinned[int(id)][1].name),
                    value="[Jump to pin]({})".format(arrayPinned[int(id)][0].message.jump_url))
...
""" discord embed generator
{
  ...
  "embed": 
  {
    ...
    "color": 4332007,
    "timestamp": "2019-11-24T16:49:07.580Z",
    "footer": 
    {
