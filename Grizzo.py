...
@bot.command(aliases=['censor'], pass_context=True, brief='Censors a word', description='Arguments: word to censor.\nFunction: adds a word to the censored list')
async def censorword(ctx, phrase):
    await ctx.send(util.censorword(phrase, ctx.message))
    pass

@bot.command(aliases=['uncensor'], pass_context=True, brief='Removes a censored word', description='Arguments: word to uncensor.\nFunction: removes a word from the censored list')
async def uncensorword(ctx, phrase):
    await ctx.send(util.uncensorword(phrase, ctx.message))
    pass

@bot.command(aliases=['listcensors'],pass_context=True , brief='shows a list of censored words', description='Arguments: none.\nFunction: displays a list of censored messages')
async def censorlist(ctx):
    await ctx.send(util.censorlist(ctx.message))
    pass

@bot.command(aliases=['removepin'], pass_context=True, brief='Removes a pin', description='Arguments: pin ID.\nRemove a spcific pin')
async def unpin(ctx, id):
    await ctx.send(await util.unpin(ctx.message, id))
    pass

@bot.command(aliases=['findpin'], pass_context=True, brief='Shows more information about a specific pin', description='Arguments: pin ID.\nFunction: displays more information about a spicific pin and lets you jump to the original message')
async def pin(ctx,id):
    await ctx.send(content="Pinned by Grizzo", embed=(await util.pin(ctx,id)))
    pass

@bot.command(aliases=['pinnedlist'], pass_context=True, brief='Shows a list of pinned messages', description='Arguments: number to start displaying pins at.\nFunction: displays a list of pins')
...
async def on_reaction_add(reaction, user):
    ...
    if reaction.emoji == "📌":
        ...
        await channel.send('{}, has pinned {}\'s message, "{}"'.format(user.mention, reaction.message.author.mention, messageContent))
