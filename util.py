...
async def listPins(ctx, index):
    ...
    for i in range(int(index), len(pinMessageIDs)):
        ...
        outputTest= output+"\n{}: {} {}, 📌 by {}:\n{}\n".format(str(i),message.author.name, message.created_at.strftime("%Y-%m-%d"),message.author.name, messageContent)
...
def DBAddCensor(serverID, phrase):
    ...
    cursor.execute("INSERT ignore INTO server (server_alias_id) SELECT * FROM (SELECT '{}') AS tmp WHERE NOT EXISTS ( SELECT server_alias_id FROM server WHERE server_alias_id='{}') LIMIT 1;".format(serverID, serverID))
    cursor.execute("INSERT ignore INTO word ( phrase) SELECT * FROM ( SELECT '{}') AS tmp WHERE NOT EXISTS ( SELECT phrase FROM word WHERE phrase='{}') LIMIT 1;".format(phrase, phrase))
    cursor.execute("Insert ignore server_word (server_id, word_id) Select server_id , word_id from server CROSS Join word where server_alias_id = '{}' and phrase = '{}';".format(serverID, phrase))
...
def DBGetCensors(serverID):
    ...
    cursor = db.cursor()
    # execute SQL query using execute() method.
    cursor.execute("select phrase from server, word, server_word where server.server_id = server_word.server_id and word.word_id = server_word.word_id and server_alias_id='{}';".format(serverID))
...
def DBRemoveCensors(serverID, phrase):
    ...
    cursor = db.cursor()
    # execute SQL query using execute() method.
    cursor.execute("DELETE from server_word where (SELECT server_id from server where server.server_alias_id='{}' ) = server_word.server_id and (SELECT word_id from word where word.phrase='{}' ) = server_word.word_id;".format(serverID,phrase))
...
def DBAddPin(message):
    ...
    # execute SQL query using execute() method.
    cursor.execute("INSERT ignore INTO server (server_alias_id) SELECT * FROM (SELECT '{}') AS tmp WHERE NOT EXISTS ( SELECT server_alias_id FROM server WHERE server_alias_id='{}') LIMIT 1;".format(message.channel.guild.id, message.channel.guild.id))
    cursor.execute("INSERT INTO pins (server_id,message_id,date,channel_id,author,content) SELECT * FROM (SELECT (select server_id from server where server.server_alias_id='{}'),'{}',DATE_FORMAT('{}', '%Y/%c/%e'),'{}','{}','{}') AS tmp WHERE NOT EXISTS ( SELECT message_id FROM pins WHERE message_id='{};') LIMIT 1;".format(message.channel.guild.id, message.id,message.created_at.strftime("%Y-%m-%d"),message.channel.id,message.author,message.content, message.id))
...
def DBGetPins(message):
    ...
    cursor.execute("select message_id from server, pins where server.server_id = pins.server_id and server_alias_id='{}' and channel_id='{}';".format(message.channel.guild.id,message.channel.id))
...
def DBUnpin(message, ID):
    ...
    cursor = db.cursor()
    # execute SQL query using execute() method.
    cursor.execute("DELETE from pins where (SELECT server_id from server where server.server_alias_id='{}' ) = pins.server_id and message_id = '{}' and channel_id='{}';".format(serverID,messageID, channelID))
